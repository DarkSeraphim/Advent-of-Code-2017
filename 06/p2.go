...
func main() {
  ...
  var val, _ = seen[toString(state[:])]
  fmt.Printf("Found a cycle after %d mutations. Cycle size was %d.\n", mutations, mutations - val)
}
...
func add(set map[string]int, key []int, when int) bool {
  var sk = toString(key)
  if _, ok := set[sk]; ok {
    return false
  }
